...
export function allocateAp(ap, initialBaseStats, initialStats, level, weapon, spell, totalWatk, rawMatk, rawWacc, monster, wDominanceFactor) {
    ...
    while (ap !== 0) {
        ...
        if (meleeDps < spellDps * wDominanceFactor) {
            ++stats.str;
            const meleeDpsStrAdd = wDps(stats, rawWacc, totalWatk, weapon, monster, d);
            --stats.str;
            ++stats.luk;
            const meleeDpsLukAdd = wDps(stats, rawWacc, totalWatk, weapon, monster, d);
            --stats.luk;
